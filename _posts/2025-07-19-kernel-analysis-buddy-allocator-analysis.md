...
categories:
  - Kernel Analysis
tags:
  - [Kernel, Memory Management, SLUB allocator, kamlloc, page]
...
이떄 페이지의 할당과 해제를 관리하는 시스템이 Buddy Allocator이다.
...
위와 똑같은 원리이므로 설명은 생략해도 될 것 같다. order 경계대로 잘린다는 사실만 잘 기억하면서 따라가보자. order-3 bitmap, order-4 bitmap도 똑같으므로 직접 생각해 보자. 전체 bitmap을 그림으로 표현하면 다음과 같다.

// 그림!

## 2-3. free_list

버디 할당자는 bitmap이외에도 `free_list`라는 array를 사용해서 실제로 사용가능한 페이지를 관리한다. 이떄 `free_list`의 인덱스는 `order`로 사용된다. 예를 들어, `free_list[0]`은
single linked list로 연결해 둔 order-0 bitmap에서 0으로 표시된 페이지(즉, 사용가능한 페이지)를 가리키고 있다. 마찬가지로 `free_list[2]`는 single linked list로 연결된 order-2 bitmap에서 사용가능으로 표시된 페이지를
가리킨다. 다시 말해, `free_list`는 bitmap으로 표현된 페이지를 실제로 리스트로 연결해둔 것이라고 생각하면 편하다.

다만 여기서도 중요한 사실이 있는데,
**<center>페이지들은 가장 큰 단위의 free_list에 단 한 번만 들어감</center>**
이다. 

예를 들어 위에서 예시로 든 상황에서 `04-07` 페이지는 4개의 인접한 페이지가 할당 가능한 상태이기 때문에 `free_list[0]`에 4개, `free_list[1]`에 2개, `free_list[2]`에 1개만 들어가는 것이
**아니라**, `free_list[2]`에 하나만 들어간다. 이는 중복 할당을 피하기 위함인데, 만약 이 페이지가 `free_list[0]`에도 들어있어서 하나의 페이지 할당 요청이 들어왔을 때 이 페이지를 할당해버리게 된다면,
이 페이지가 할당받았음에도 불구하고 2개의 페이지 할당 요청이나 4개의 페이지 할당 요청이 들어온 후 1개의 페이지 할당 요청이 들어왔을 때 재할당될 가능성이 존재하기 때문이다.

위와 같은 페이지 상황일 때, 위에서 말한 규칙을 고려해 `free_list`를 그림으로 표현하면 다음과 같다. 
...
앞서 말했듯 버디 할당자는 외부 단편화 최소를 위해 물리적으로 연속된 페이지를 할당하려고 한다. 이때 2개의 연속된 할당가능한 페이지는 order-1과 관계있고, 즉 `free_list[1]`을 탐색하면 쉽게 찾아낼 수 있다. 
...
order-1 bitmap에서 관찰 가능하듯 `04-05` 페이지가 할당 가능한 상태로 표시되어 있기 때문에 이 패이지들을 반환하고, 두 패이지 모두 1로 표시해 중복 할당이 일어나지 않도록 한다. 할당한 이후 order-0 bitmap부터
order-4 bitmap까지 다시 그림으로 표현해 보면 다음과 같다.

위에서 bitmap으로 설명한 것은 이해를 돕기 위함이고, 사실 버디 할당자는 bitmap을 기준으로 페이지를 할당하지는 않는다. 대신 위에서 언급했던 `free_list`를 사용하는데, 여기서는 order-1 페이지를 할당해야 하기 때문에
`free_list[1]`을 탐색해 사용 가능한 페이지를 찾고, 그 페이지를 반환한다. 이때 페이지 번호 `04-05`가 할당처리되며, 나머지 order에 대한 `free_list`의 갱신이 이루어진다. 
할당이 일어난 후 전체 `order`에 대한 `bitmap`과 `free_list`를 그림으로 표현하면 다음과 같다. 

// 그림!

## 3-2. 4 page 할당 요청

4 페이지에 대한 할당 요청이 일어나면 당연히 `free_list[2]`에 대한 탐색이 이루어진다. 이떄 페이지 번호 `04-07`에 대한 할당이 가능하기 때문에 이 페이지를 반환하고 반환된 페이지들은
할당 불가능으로 표시되며 다시 한 번 전체 `free_list`에 대한 갱신이 이루어진다. 그림으로 표현하면 다음과 같다.

// 그림!

## 3-3. `free_list[N]`에서 빈 페이지를 찾을 수 없을 때
...
이런 상황에서 2개의 페이지에 대한 할당 요청이 들어온다면, `free_list[1]`은 비어있기 때문에 `free_list[1]`에 대한 검색은 실패할 것이다. (이는 위에서 말한 규칙을 잘 생각해보며
order-N bitmap을 잘 그려보면 이해할 수 있다. 이에 대한 설명은 생략해도 될 것 같다.)

이렇게 원하는 `free_list`에 할당 가능한 페이지가 없을 떄 버디 할당자는 하나 높은 `free_list`를 탐색한다. 즉 위의 상황에서는 `free_list[1]`에 대한 탐색이 실패했기 때문에
`free_list[2]`에 대한 탐색을 시도하고, `free_list[2]`에는 `04-07`에 해당하는 페이지가 존재하기 때문에 버디 할당자는 이 페이지들을 분할해 사용하게 된다. 즉

1. `04-07`에 해당하는 페이지를 `free_list[2]`에서 삭제함
2. `04-05`에 해당하는 페이지 중 `04-05`애 해당하는 페이지를 반환함
3. `06-07`에 해당하는 페이지를 `free_list[1]`에 추가함

위와 같은 동작을 수행하게 된다. 이러한 작업을 **분할**이라고 한다. 
...
버디 할당자는 이러한 상황이 발생했을 때 `01` 페이지를 `free_list[0]`에서 삭제하고 `00-01`을 `free_list[1]`에 추가한다. 이후 `03` 페이지에 대한 해제 요청이 들어왔다고 가정하자.
이때 버디 할당자는 원래 `free_list[1]`에 들어 있던 `00-01` 페이지들을 삭제하고 `free_list[2]`에 `00-03`을 추가하려고 시도한다. 넓게 보면 `00-07`까지의 페이지가 전부 할당 가능한 상태이므로
원래 `free_list[2]`에 들어 있던 `04-07` 페이지를 제거하고 새로 `free_list[3]`에 `00-07` 페이지 전체를 추가한다.

이렇게 페이지들을 합치는 과정을 **병합**이라고 한다. 
