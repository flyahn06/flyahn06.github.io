...
위와 같은 페이지 상황일 때, 위에서 말한 규칙을 고려해 `free_list`를 표현하면 다음과 같다. 

```c
free_list[0] = {1, 2, 8, 10}
free_list[1] = {14}
free_list[2] = {4}
free_list[3] = {}
free_list[4] = {}
...
order-1 bitmap에서 관찰 가능하듯 `04-05` 페이지가 할당 가능한 상태로 표시되어 있으나, 이 page는 이미 `free_list[2]`에 속한 상태이다. 즉 `free_list[1]`에는 `14-15` 페이지만 존재하므로, 이 페이지를 반환한다.
...
order-N bitmap과 `free_list` 잘 그려보면 이해할 수 있다. 이에 대한 설명은 생략해도 될 것 같다.)
...
위와 같은 동작을 수행하게 된다. 이러한 작업을 **분할**이라고 한다. 이렇게 분할이 일어나며 `free_list`는 다음과 같이 갱신될 것이다. (여기서는 `free_list[2]`까지만 표현했다.)
...
이렇게 페이지들을 합치는 과정을 **병합**이라고 한다. 이렇게 분할이 일어나며 `free_list`는 다음과 같이 갱신될 것이다. (여기서는 `free_list[3]`까지만 표현했다.)
